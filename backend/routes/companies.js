...
router.post('/', authorize('admin'), audit('CREATE', 'company'), async (req, res) => {
  try {
...
router.put('/:id', authorize('admin'), audit('UPDATE', 'company'), async (req, res) => {
  try {
    const { name, code, description } = req.body;
...
router.delete('/:id', authorize('admin'), audit('DELETE', 'company'), async (req, res) => {
  try {
