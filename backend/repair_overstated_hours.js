 ...
 *
 *    sessions   SUM(session_hours) is what was actually worked, gaps excluded.
 *               Authoritative when sessions exist.
 *    span       check_out - check_in. Right for a single unbroken stretch,
 *               and the best available when there are no sessions.
 *
...
(async () => {
  ...
  const rows = (await pool.query(
    `SELECT a.id, e.employee_id AS code, TRIM(CONCAT(e.first_name,' ',e.last_name)) AS name,
            ...
            EXTRACT(EPOCH FROM (a.check_out - a.check_in))/3600.0 AS span_h,
      ...
      WHERE a.check_in IS NOT NULL AND a.check_out IS NOT NULL
        -- More hours than the punches can account for. The span is the outer
        -- bound of a day: nothing worked can exceed it.
        AND COALESCE(a.working_hours, 0)
            - EXTRACT(EPOCH FROM (a.check_out - a.check_in))/3600.0 > $1
  ...
  for (const r of rows) {
    ...
    if (r.sessions > 0 && sessionSum > 0 && sessionSum - span <= TOLERANCE_H) {
      corrected = sessionSum; source = `${r.sessions} session(s)`;
    } else if (r.sessions > 0 && sessionSum - span > TOLERANCE_H) {
  ...
  if (fixable.length) {
    ...
    for (const { r, stored, corrected, source } of fixable) {
      ...
      console.log(`    ${r.code.padEnd(14)} ${r.d}   ${clock(r.check_in)} - ${clock(r.check_out)}`);
