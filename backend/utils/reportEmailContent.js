...
/* One shared shell so all eight emails look like they belong to the same
 * feature. `rows` is an array of [label, value] pairs rendered as a simple
 * stat table; `table` is an optional pre-built HTML table for per-employee
 * detail beneath it. */
function wrap({ title, periodLabel, introLine, rows = [], table = '', linkPath, linkLabel }) {
...
<body style="margin:0;padding:24px;background:#f1f5f9;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif;">
  <table role="presentation" cellpadding="0" cellspacing="0" style="max-width:600px;margin:0 auto;background:#ffffff;border-radius:12px;overflow:hidden;border:1px solid #e2e8f0;">
    ...
    ${introLine ? `<tr><td style="padding:0 32px 16px;font-size:13.5px;color:#334155;">${escapeHtml(introLine)}</td></tr>` : ''}
...
async function dailyAttendanceEmail(reportDate) {
...
async function weeklyAttendanceEmail(start, end, holidays) {
  const r = await pool.query(
...
async function onboardingEmail(start, end) {
  const r = await pool.query(
...
async function musterRollEmail(start, end) {
  const r = await pool.query(
...
async function monthlyAttendanceEmail(start, end) {
  const r = await pool.query(
    `SELECT
       (SELECT COUNT(*) FROM employees WHERE status='active' AND deleted_at IS NULL AND is_user=TRUE) AS total,
       (SELECT COUNT(*) FROM attendance a JOIN employees e ON e.id=a.employee_id
...
async function payrollFeedEmail(start, end) {
...
async function lopDataEmail(start, end) {
  const lop = await lopByEmployee(start, end);
  const total = lop.reduce((s, r) => s + r.days, 0);
  return wrap({
...
async function regularizationReminderEmail(recipientId, isFullAccessRole) {
  const r = await pool.query(
