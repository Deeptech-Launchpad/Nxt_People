 ...
 */
const { pickRule, resolveLevels } = require('./approvalRules');

async function deriveLevels(db, employeeId, requestType, context = {}) {
  ...
  // The configured approval wins. Which one applies depends on the request's
  // own fields, so a rule narrowed by criteria can route differently from the
  // catch-all beneath it. The seeded rules carry no criteria and reproduce the
  // chain below exactly, so this changes nothing until one is added.
  const rule = await pickRule(db, requestType, context);
  if (rule) {
    // Auto approve and auto reject settle the request without anyone in the
    // chain, so there are no levels to create.
    if (rule.decision === 'auto_approve' || rule.decision === 'auto_reject') return [];
    return resolveLevels(db, employeeId, ancestors, rule.levels);
  }
