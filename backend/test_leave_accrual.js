...
//   3 January and Zoho grants him 3.74 hours for it: 4 × 29/31. We granted the
//   month whole.
//
// The decision was to match Zoho, so these are the numbers the reference gives
// and this asserts them exactly rather than approximately — 3.74 is not "about
// four".
...
console.log('\n════ The joining month is charged by the day ════\n');

// Balaji: joined 3 January 2026, 4 hours a month. Zoho grants 47.74.
const balaji = total(monthly(4), { joiningDate: '2026-01-03' });
check('joining on 3 January grants 47.74 hours, exactly as the reference does',
  balaji === 47.74, balaji);
...
check('January is 3.74 — four hours × 29 of its 31 days',
  events[0].amount === 3.74, events[0]);
check('February is whole', events[1].amount === 4, events[1]);
check('and it is dated the joining day, not the 1st',
  events[0].date === '2026-01-03', events[0].date);

check('joining on the 1st is not prorated at all',
  total(monthly(4), { joiningDate: '2026-01-01' }) === 48,
  total(monthly(4), { joiningDate: '2026-01-01' }));

// February has 28 days in 2026, so a mid-month join there proves the divisor
// is the month's own length and not a hardcoded 30 or 31.
const feb = accrualEvents(monthly(4), { year: 2026, joiningDate: '2026-02-15' });
check('February the 15th grants 4 × 14/28 = 2',
  feb[0].amount === 2, feb[0]);
...
console.log('\n════ Shivanie and Balaji, as the reference shows them ════\n');

check('Shivanie: 48 granted − 11.5 taken = 36.5',
  Math.round((total(monthly(4), { joiningDate: '2023-11-01' }) - 11.5) * 100) / 100 === 36.5);
check('Balaji: 47.74 granted − 2 taken = 45.74',
  Math.round((total(monthly(4), { joiningDate: '2026-01-03' }) - 2) * 100) / 100 === 45.74);
