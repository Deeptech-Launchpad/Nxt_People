 ...
 *  6-role model stays consistent across the app.
 *
 *    admin, director, hr_admin → full access (sees & manages everything)
...
export const isFullAccess = (u) =>
  [ROLES.ADMIN, ROLES.DIRECTOR, ROLES.HR_ADMIN].includes(roleOf(u));

/** Anyone who can reach an approver screen (full access or a team manager). */
export const isApprover = (u) =>
  [ROLES.ADMIN, ROLES.DIRECTOR, ROLES.HR_ADMIN, ROLES.MANAGER, ROLES.TEAM_INCHARGE].includes(roleOf(u));
...
export const ASSIGNABLE_ROLES = [
  ROLES.ADMIN, ROLES.DIRECTOR, ROLES.HR_ADMIN,
