...
export default function EmployeeDashboard() {
  ...
  return (
    <ReportShell title="Employee Dashboard" subtitle="Headcount snapshot as of now" loading={loading}>
      {data && (
        <div className="p-5 space-y-5">
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
            <StatCard label="Active Employees" value={data.active} color="text-emerald-700" />
            <StatCard label="Inactive" value={data.inactive} color="text-slate-500" />
            <StatCard label="New This Month" value={data.newThisMonth} color="text-blue-700" />
            <StatCard label="Exits This Month" value={data.exitsThisMonth} color="text-red-600" />
          </div>
