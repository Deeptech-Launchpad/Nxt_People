    ...
    <div className="relative" ref={ref}>
      ...
      {open && (
        <div className="absolute z-50 mt-1 w-52 bg-white border border-slate-200 rounded-lg shadow-lg py-1 whitespace-normal">
          {TYPES.map(([k, l]) => (
            <button key={k} onClick={() => { setBy(k); setOpen(false); }} className={`block w-full text-left px-4 py-2 text-[13px] transition-colors ${by === k ? 'bg-blue-50 text-blue-600 font-semibold' : 'text-slate-700 hover:bg-slate-50'}`}>{l}</button>
          ))}
        </div>
      )}
    </div>
