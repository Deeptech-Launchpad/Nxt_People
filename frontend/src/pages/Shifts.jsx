              ...
              <div className="p-5">
                <div className="flex items-start justify-between">
                  <div>
                    ...
                    {s.isDefault && <span className="text-sm bg-brand-50 text-brand-600 px-2 py-0.5 rounded-full mt-1 inline-block">Default</span>}
                  </div>
                ...
                <div className="flex gap-1.5 mt-4 flex-wrap">
                  {DAYS.map(d => (
                    <span key={d} className={`text-sm px-2 py-1 rounded-lg font-medium ${s.workingDays?.includes(d) ? 'text-white' : 'bg-slate-100 text-slate-400'}`} style={s.workingDays?.includes(d) ? {backgroundColor:s.color} : {}}>
                      {d}
                    </span>
                  ))}
                </div>
        ...
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl w-full max-w-md shadow-2xl">
            <div className="flex items-center justify-between p-6 border-b border-slate-100">
              <h3 className="font-display font-semibold text-slate-800 text-xl">{editShift ? 'Edit Shift' : 'Create Shift'}</h3>
            ...
            <form onSubmit={handleSave} className="p-6 space-y-4">
              <div>
                <label className="block text-sm font-medium text-slate-600 mb-1.5">Shift Name</label>
                <input value={form.name} onChange={e=>setForm({...form,name:e.target.value})} required placeholder="e.g. General Shift" className="w-full border border-slate-200 rounded-xl px-3 py-2.5 text-base focus:outline-none focus:border-brand-400 focus:ring-1 focus:ring-brand-400"/>
              </div>
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-sm font-medium text-slate-600 mb-1.5">Start Time</label>
                  <input type="time" value={form.startTime} onChange={e=>setForm({...form,startTime:e.target.value})} className="w-full border border-slate-200 rounded-xl px-3 py-2.5 text-base focus:outline-none focus:border-brand-400"/>
                </div>
                <div>
                ...
                <div className="flex gap-2 flex-wrap">
                  {DAYS.map(d => (
                    <button key={d} type="button" onClick={() => toggleDay(d)}
                      className={`px-3 py-1.5 rounded-lg text-sm font-medium transition-colors ${form.workingDays.includes(d) ? 'bg-brand-600 text-white' : 'bg-slate-100 text-slate-600 hover:bg-slate-200'}`}>
                      {d}
                    </button>
                  ))}
                </div>
