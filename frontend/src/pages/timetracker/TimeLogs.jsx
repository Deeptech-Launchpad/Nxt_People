  ...
  return (
    <div className="p-6 max-w-5xl space-y-5">
      ...
      <div className="flex items-center gap-3">
        ...
        <span className="text-[13px] font-semibold text-slate-700">{weekLabel}</span>
        <button onClick={() => setWeek(w => w + 1)} className="w-7 h-7 border border-slate-200 rounded hover:bg-slate-50 flex items-center justify-center text-slate-500">
          <ChevronRight size={14}/>
        </button>
        <button onClick={() => setWeek(0)} className="text-[12px] text-blue-600 hover:underline ml-1">Today</button>
      </div>

      {/* Add log form */}
      <form onSubmit={handleAdd} className="bg-white rounded-lg border border-slate-200 p-4 shadow-sm">
        <p className="text-[13px] font-semibold text-slate-700 mb-3">Add Time Log</p>
        <div className="grid grid-cols-2 sm:grid-cols-5 gap-2">
          <input type="date" value={form.date} onChange={e => setForm({...form, date: e.target.value})}
            className="border border-slate-200 rounded-lg px-3 py-2 text-[12.5px] focus:outline-none focus:border-blue-400"/>
          ...
          <button type="submit" disabled={saving}
            className="bg-blue-600 hover:bg-blue-700 text-white rounded-lg px-4 py-2 text-[12.5px] font-semibold flex items-center justify-center gap-1.5 disabled:opacity-60">
            <Plus size={13}/> Add
          </button>
        </div>
      </form>
        ...
        <table className="w-full">
          ...
          <tbody className="divide-y divide-slate-50">
            {loading ? (
              ...
              <tr><td colSpan={5} className="py-12 text-center text-[13px] text-slate-400">No time logs for this week</td></tr>
            ) : logs.flatMap(ts => ts.entries || []).map((e, i) => (
              <tr key={i} className="hover:bg-slate-50 transition-colors">
                <td className="px-4 py-3 text-[12.5px] text-slate-600">{new Date(e.date).toLocaleDateString('en-IN',{day:'2-digit',month:'short'})}</td>
                <td className="px-4 py-3 text-[12.5px] font-medium text-slate-700">{e.project}</td>
                <td className="px-4 py-3 text-[12.5px] text-slate-600">{e.task}</td>
                <td className="px-4 py-3 text-[12.5px] font-bold text-blue-600">{e.hours}h</td>
                <td className="px-4 py-3 text-[12.5px] text-slate-400">{e.description}</td>
              </tr>
            ))}
          </tbody>
        </table>
