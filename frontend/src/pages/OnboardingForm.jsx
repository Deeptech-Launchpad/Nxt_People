...
const Input = ({ label, name, type = 'text', required = false, placeholder = '', value, onChange }) => (
  <div>
    ...
    <input type={type} name={name} value={value || ''} onChange={onChange} placeholder={placeholder} className="w-full bg-white border border-slate-300 text-slate-900 px-4 py-2.5 rounded-xl text-base focus:outline-none focus:border-brand-500 focus:ring-1 focus:ring-brand-500" />
  </div>
);
...
const Select = ({ label, name, options, required = false, value, onChange }) => (
  <div>
    <label className="block text-base font-medium text-slate-700 mb-1.5">{label} {required && <span className="text-red-500">*</span>}</label>
    <select name={name} value={value || ''} onChange={onChange} className="w-full bg-white border border-slate-300 text-slate-900 px-4 py-2.5 rounded-xl text-base focus:outline-none focus:border-brand-500 focus:ring-1 focus:ring-brand-500">
...
const FileInput = ({ label, name, required = false, accept = ".pdf,.jpg,.jpeg,.png", file, onChange }) => (
  <div>
    ...
    <div className="relative border-2 border-dashed border-slate-300 rounded-xl p-4 text-center hover:bg-slate-50 transition-colors">
      ...
      {file ? (
        <div className="flex flex-col items-center gap-1 text-emerald-600"><CheckCircle2 size={24}/> <span className="text-base font-medium truncate w-full px-4">{file.name}</span></div>
      ) : (
        <div className="flex flex-col items-center gap-1 text-slate-500"><Upload size={24} className="text-slate-400"/> <span className="text-base">Click to upload or drag & drop</span><span className="text-sm text-slate-400">Max 5MB</span></div>
      )}
    </div>
  </div>
);
  ...
  if (submitted) return (
    <div className="min-h-screen flex items-center justify-center bg-slate-50 p-6 text-center">
      <div className="bg-white p-10 rounded-2xl shadow-sm border border-slate-200 max-w-lg w-full">
        ...
        <h2 className="text-3xl font-display font-bold text-slate-800 mb-3">Submitted Successfully</h2>
        <p className="text-slate-600 text-xl">Your details have been submitted successfully. HR will review your information and get in touch with you.</p>
      </div>
    </div>
  );
    ...
    <div className="min-h-screen bg-slate-50 py-10 px-4">
      <div className="max-w-4xl mx-auto">
        <div className="flex items-center gap-3 mb-8 justify-center">
          ...
          <h1 className="text-3xl font-display font-bold text-slate-800 tracking-tight">Onboarding Portal</h1>
        </div>
          ...
          <div className="p-6 sm:p-8">
            ...
            {step === 1 && (
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                ...
                <div>
                  <label className="block text-base font-medium text-slate-700 mb-1.5">Personal Email <span className="text-red-500">*</span></label>
                  <input type="text" value={email} disabled className="w-full bg-slate-100 border border-slate-300 text-slate-500 px-4 py-2.5 rounded-xl text-base cursor-not-allowed" />
                </div>
            ...
            {step === 2 && (
              <div className="space-y-6">
                <div>
                  <label className="block text-base font-medium text-slate-700 mb-1.5">Current Address <span className="text-red-500">*</span></label>
                  <textarea name="currentAddress" value={form.currentAddress} onChange={handleInputChange} rows={3} className="w-full bg-white border border-slate-300 text-slate-900 px-4 py-2.5 rounded-xl text-base focus:outline-none focus:border-brand-500 focus:ring-1 focus:ring-brand-500 resize-none"></textarea>
                </div>
                <label className="flex items-center gap-2 cursor-pointer">
                  ...
                  <span className="text-base text-slate-700 font-medium">Same as current address</span>
                </label>
                <div>
                  <label className="block text-base font-medium text-slate-700 mb-1.5">Permanent Address <span className="text-red-500">*</span></label>
                  <textarea name="permanentAddress" value={form.permanentAddress} onChange={handleInputChange} rows={3} disabled={form.sameAsCurrent} className={`w-full border border-slate-300 text-slate-900 px-4 py-2.5 rounded-xl text-base focus:outline-none focus:border-brand-500 focus:ring-1 focus:ring-brand-500 resize-none ${form.sameAsCurrent ? 'bg-slate-100' : 'bg-white'}`}></textarea>
                </div>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
            ...
            {step === 3 && (
              <div className="space-y-6">
                <div className="bg-amber-50 text-amber-700 p-4 rounded-xl text-base mb-6 border border-amber-200">
                ...
                {education.map((edu, idx) => (
                  <div key={idx} className="p-5 border border-slate-200 rounded-xl bg-slate-50 relative">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
                      <div>
                        <label className="block text-base font-medium text-slate-700 mb-1.5">Highest Qualification <span className="text-red-500">*</span></label>
                        <select value={edu.highestQualification} onChange={(e) => handleEduChange(idx, 'highestQualification', e.target.value)} required className="w-full bg-white border border-slate-300 text-slate-900 px-4 py-2.5 rounded-xl text-base focus:outline-none focus:border-brand-500 focus:ring-1 focus:ring-brand-500">
                          <option value="">Select...</option>
                          {['10th', '12th', 'Diploma', 'UG', 'PG', 'PhD', 'Other'].map(o => <option key={o} value={o}>{o}</option>)}
                        </select>
                      </div>
                      <div>
                        <label className="block text-base font-medium text-slate-700 mb-1.5">Degree <span className="text-red-500">*</span></label>
                        <input type="text" value={edu.degree} onChange={(e) => handleEduChange(idx, 'degree', e.target.value)} required placeholder="e.g. B.Tech, B.Sc" className="w-full bg-white border border-slate-300 text-slate-900 px-4 py-2.5 rounded-xl text-base focus:outline-none focus:border-brand-500 focus:ring-1 focus:ring-brand-500" />
                      </div>
                      <div className="sm:col-span-2">
                        <label className="block text-base font-medium text-slate-700 mb-1.5">Course / Specialization <span className="text-red-500">*</span></label>
                        <input type="text" value={edu.course} onChange={(e) => handleEduChange(idx, 'course', e.target.value)} required placeholder="e.g. Artificial Intelligence and Data Science, Computer Science" className="w-full bg-white border border-slate-300 text-slate-900 px-4 py-2.5 rounded-xl text-base focus:outline-none focus:border-brand-500 focus:ring-1 focus:ring-brand-500" />
                      </div>
                      <div>
                        <label className="block text-base font-medium text-slate-700 mb-1.5">University / Institution <span className="text-red-500">*</span></label>
                        <input type="text" value={edu.universityOrInstitution} onChange={(e) => handleEduChange(idx, 'universityOrInstitution', e.target.value)} required className="w-full bg-white border border-slate-300 text-slate-900 px-4 py-2.5 rounded-xl text-base focus:outline-none focus:border-brand-500 focus:ring-1 focus:ring-brand-500" />
                      </div>
                      <div>
                        <label className="block text-base font-medium text-slate-700 mb-1.5">Year of Passing <span className="text-red-500">*</span></label>
                      ...
                      <div>
                        <label className="block text-base font-medium text-slate-700 mb-1.5">Percentage / CGPA <span className="text-red-500">*</span></label>
