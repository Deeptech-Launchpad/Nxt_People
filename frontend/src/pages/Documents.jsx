    ...
    <div className="space-y-5">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-slate-400 text-base mt-0.5">{docs.length} document{docs.length !== 1 ? 's' : ''} · PDF, Word, Images, Excel supported</p>
        </div>
        {canUpload && (
      ...
      {loading ? (
        ...
        <div className="bg-white rounded-2xl border border-slate-100 shadow-sm text-center py-20">
          ...
          {canUpload && <p className="text-slate-300 text-base mt-1">Click Upload to add your first document</p>}
        </div>
      ) : (
          ...
          {DOC_TYPES.filter(t => grouped[t]?.length).map((type) => {
            ...
            <div key={type}>
              <div className="flex items-center gap-3 mb-3">
                <h3 className="font-display font-semibold text-slate-700 text-base">{DOC_TYPE_LABELS[type] || type}</h3>
                <span className="text-sm text-slate-400 bg-slate-100 px-2 py-0.5 rounded-full">{typeDocs.length}</span>
                  ...
                  return (
                    <div key={doc._id} className="bg-white rounded-2xl border border-slate-100 shadow-sm p-4 flex items-start gap-3 group hover:shadow-md transition-shadow">
                      ...
                      <div className="flex-1 min-w-0">
                        <p className="font-medium text-slate-700 text-base truncate">{doc.name}</p>
        ...
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl w-full max-w-md shadow-2xl">
            <div className="flex items-center justify-between p-6 border-b border-slate-100">
              <h3 className="font-display font-semibold text-slate-800 text-xl">Upload Document</h3>
            ...
            <form onSubmit={handleUpload} className="p-6 space-y-4">
              <div>
                <label className="block text-sm font-medium text-slate-600 mb-1.5">Document Name</label>
                <input value={form.name} onChange={e => setForm({ ...form, name: e.target.value })} placeholder="e.g., Offer Letter 2024" className="w-full border border-slate-200 rounded-xl px-3 py-2.5 text-base focus:outline-none focus:border-brand-400" />
              </div>
              <div>
                <label className="block text-sm font-medium text-slate-600 mb-1.5">Document Type</label>
                <select value={form.type} onChange={e => setForm({ ...form, type: e.target.value })} className="w-full border border-slate-200 rounded-xl px-3 py-2.5 text-base focus:outline-none focus:border-brand-400">
                  {DOC_TYPES.map(t => <option key={t} value={t}>{DOC_TYPE_LABELS[t]}</option>)}
                </select>
              </div>
              <div>
                <label className="block text-sm font-medium text-slate-600 mb-1.5">File * <span className="text-slate-400">(PDF, Word, Image, Excel — max 10MB)</span></label>
                <div className="border-2 border-dashed border-slate-200 rounded-xl p-6 text-center hover:border-brand-300 transition-colors cursor-pointer" onClick={() => fileRef.current?.click()}>
                  {form.file ? (
                    <div><FileText size={24} className="text-brand-500 mx-auto mb-2" /><p className="text-base font-medium text-slate-700">{form.file.name}</p><p className="text-sm text-slate-400">{formatSize(form.file.size)}</p></div>
                  ) : (
                    <div><Upload size={24} className="text-slate-300 mx-auto mb-2" /><p className="text-base text-slate-400">Click to choose file</p></div>
                  )}
