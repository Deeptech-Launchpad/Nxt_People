    ...
    <div className="space-y-5">
      ...
      <div className="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden">
        <div className="flex items-center justify-between p-5 border-b border-slate-100">
          <div>
            ...
            <p className="text-slate-400 text-sm mt-0.5">Submit your resignation and track clearance status</p>
          </div>
          {!myExit && (
            <button onClick={() => setModal(true)} className="flex items-center gap-2 bg-red-500 hover:bg-red-400 text-white px-4 py-2.5 rounded-xl text-sm font-medium transition-colors shadow-sm shadow-red-400/25">
        ...
        {loading ? (
          ...
          <div className="text-center py-16">
            ...
            <p className="text-slate-300 text-sm mt-1">Click "Submit Resignation" to begin the exit process</p>
          </div>
        ) : (
          <div className="p-6">
            <div className="flex items-start justify-between mb-5">
              <div>
                <p className="text-sm text-slate-500">Resignation Date: <span className="font-medium text-slate-700">{new Date(myExit.resignationDate + 'T00:00:00').toLocaleDateString('en-IN', { day: 'numeric', month: 'long', year: 'numeric' })}</span></p>
                {myExit.lastWorkingDate && <p className="text-sm text-slate-500 mt-0.5">Last Working Day: <span className="font-medium text-slate-700">{new Date(myExit.lastWorkingDate + 'T00:00:00').toLocaleDateString('en-IN', { day: 'numeric', month: 'long', year: 'numeric' })}</span></p>}
                <p className="text-sm text-slate-500 mt-0.5">Reason: <span className="text-slate-600">{myExit.reason}</span></p>
              </div>
              <span className={`text-xs px-3 py-1.5 rounded-full font-semibold capitalize ${STATUS_STYLE[myExit.status]}`}>{myExit.status}</span>
            </div>
            {myExit.status === 'approved' && (
              <div>
                <p className="text-xs font-semibold text-slate-500 uppercase tracking-wider mb-3 flex items-center gap-2"><Shield size={12} /> Clearance Checklist</p>
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                  {[['IT', myExit.itClearance], ['HR', myExit.hrClearance], ['Finance', myExit.financeClearance], ['Manager', myExit.managerClearance]].map(([label, done]) => (
                    <div key={label} className={`flex items-center gap-2 p-3 rounded-xl border ${done ? 'border-emerald-200 bg-emerald-50' : 'border-slate-200 bg-slate-50'}`}>
                      ...
                      <span className={`text-sm font-medium ${done ? 'text-emerald-700' : 'text-slate-500'}`}>{label}</span>
                    </div>
                  ))}
                </div>
                {myExit.exitInterviewNotes && (
                  <div className="mt-3 bg-slate-50 rounded-xl p-3 border border-slate-100">
                    <p className="text-xs font-semibold text-slate-500 mb-1">Exit Interview Notes</p>
                    <p className="text-sm text-slate-600">{myExit.exitInterviewNotes}</p>
                  </div>
                )}
              </div>
            )}
      ...
      {user?.role === 'admin' && (
        <div className="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden">
          <div className="p-5 border-b border-slate-100">
            ...
            <p className="text-slate-400 text-sm mt-0.5">{allExits.length} total exit request{allExits.length !== 1 ? 's' : ''}</p>
          </div>
          {allExits.length === 0 ? (
            <div className="text-center py-12 text-slate-300 text-sm">No exit requests yet</div>
          ) : (
                  ...
                  <div className="flex items-start justify-between gap-4">
                    <div className="flex items-start gap-4">
                      ...
                      <div>
                        ...
                        <p className="text-xs text-slate-400">{ex.employee?.designation} · {ex.employee?.department} · {ex.employee?.employeeId}</p>
                        <p className="text-sm text-slate-500 mt-1">Resigned: {new Date(ex.resignationDate + 'T00:00:00').toLocaleDateString('en-IN', { day: 'numeric', month: 'short', year: 'numeric' })}</p>
                        <p className="text-xs text-slate-400 mt-0.5 max-w-sm">{ex.reason}</p>
                        {ex.status === 'approved' && (
                          <div className="flex gap-2 mt-2">
                            {[['IT', ex.itClearance], ['HR', ex.hrClearance], ['Finance', ex.financeClearance], ['Mgr', ex.managerClearance]].map(([l, d]) => (
                              <span key={l} className={`text-[10px] px-2 py-0.5 rounded-full font-medium ${d ? 'bg-emerald-100 text-emerald-700' : 'bg-slate-100 text-slate-400'}`}>{l}</span>
                            ))}
                          </div>
                        )}
                      </div>
                    ...
                    <div className="flex items-center gap-2 flex-shrink-0 flex-col sm:flex-row">
                      <span className={`text-xs px-2.5 py-1 rounded-full font-medium capitalize ${STATUS_STYLE[ex.status]}`}>{ex.status}</span>
                      {ex.status === 'pending' && (
                        <>
                          <button onClick={() => handleAction(ex._id, 'approved')} disabled={actionLoading} className="flex items-center gap-1.5 bg-emerald-50 text-emerald-600 hover:bg-emerald-100 px-3 py-1.5 rounded-lg text-xs font-medium transition-colors">
                            <CheckCircle size={13} /> Accept
                          </button>
                          <button onClick={() => { setRejectModal(ex._id); setRejectReason(''); }} className="flex items-center gap-1.5 bg-red-50 text-red-500 hover:bg-red-100 px-3 py-1.5 rounded-lg text-xs font-medium transition-colors">
                            <XCircle size={13} /> Reject
                          </button>
                        </>
                      )}
                      {ex.status === 'approved' && (
                        <button onClick={() => openClearance(ex)} className="flex items-center gap-1.5 bg-brand-50 text-brand-600 hover:bg-brand-100 px-3 py-1.5 rounded-lg text-xs font-medium transition-colors">
        ...
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl w-full max-w-md shadow-2xl">
            <div className="flex items-center justify-between p-6 border-b border-slate-100">
              <h3 className="font-display font-semibold text-slate-800 text-lg">Submit Resignation</h3>
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-4">
              ...
              <div>
                ...
                <input type="date" value={form.resignationDate} onChange={e => setForm({ ...form, resignationDate: e.target.value })} required
                  min={new Date().toLocaleDateString('en-CA')} className="w-full border border-slate-200 rounded-xl px-3 py-2.5 text-sm focus:outline-none focus:border-brand-400" />
              </div>
              <div>
                <label className="block text-xs font-medium text-slate-600 mb-1.5">Proposed Last Working Day</label>
              ...
              <div>
                ...
                <textarea value={form.reason} onChange={e => setForm({ ...form, reason: e.target.value })} required rows={4}
                  placeholder="Please state your reason for resignation..." className="w-full border border-slate-200 rounded-xl px-3 py-2.5 text-sm focus:outline-none focus:border-brand-400 resize-none" />
              </div>
              <div className="flex gap-3 pt-1">
                ...
                <button type="submit" disabled={saving} className="flex-1 bg-red-500 hover:bg-red-400 text-white py-2.5 rounded-xl text-sm font-medium transition-colors disabled:opacity-60 flex items-center justify-center gap-2">
          ...
          <div className="bg-white rounded-2xl w-full max-w-sm shadow-2xl p-6">
            ...
            <div className="flex gap-3">
              ...
              <button onClick={() => handleAction(rejectModal, 'rejected', rejectReason)} disabled={actionLoading}
                className="flex-1 bg-red-500 hover:bg-red-400 text-white py-2.5 rounded-xl text-sm font-medium transition-colors disabled:opacity-60">Confirm</button>
            </div>
          </div>
        </div>
      )}
      ...
      {selectedExit && (
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl w-full max-w-md shadow-2xl">
            <div className="flex items-center justify-between p-6 border-b border-slate-100">
              <h3 className="font-display font-semibold text-slate-800 text-lg">Clearance — {selectedExit.employee?.firstName} {selectedExit.employee?.lastName}</h3>
            ...
            <div className="p-6 space-y-3">
              ...
              <div>
                <label className="block text-xs font-medium text-slate-600 mb-1.5">Exit Interview Notes</label>
                <textarea value={clearance.exitInterviewNotes || ''} onChange={e => setClearance(c => ({ ...c, exitInterviewNotes: e.target.value }))} rows={3}
                  placeholder="Notes from exit interview..." className="w-full border border-slate-200 rounded-xl px-3 py-2.5 text-sm focus:outline-none focus:border-brand-400 resize-none" />
              </div>
              <div className="flex gap-3 pt-1">
                <button onClick={() => setSelectedExit(null)} className="flex-1 border border-slate-200 text-slate-600 py-2.5 rounded-xl text-sm font-medium hover:bg-slate-50">Cancel</button>
                <button onClick={() => handleClearance(selectedExit._id)} disabled={actionLoading}
                  className="flex-1 bg-brand-600 hover:bg-brand-500 text-white py-2.5 rounded-xl text-sm font-medium transition-colors disabled:opacity-60 flex items-center justify-center gap-2">
