          ...
          return (
            <div key={d.id}>
              <button onClick={() => setExpanded(isOpen ? null : d.id)} className="w-full px-4 py-3 flex items-center justify-between hover:bg-slate-50 text-left">
                <div className="flex items-center gap-3">
                  <div className="w-9 h-9 rounded-full bg-blue-50 text-blue-600 flex items-center justify-center font-bold text-[15px]">
                ...
                <div className="flex items-center gap-3">
                  <div className="text-right">
                    <p className="text-[13px] uppercase tracking-wider text-slate-400 font-bold">Total claimed</p>
                    <p className="text-[16px] font-bold text-blue-700">{fmtINR(total)}</p>
                  </div>
              ...
              {isOpen && (
                <div className="bg-slate-50 px-4 py-4 border-t border-slate-100">
                  {d.regime === 'new' ? (
                    <p className="text-[14px] text-slate-600 italic">Employee chose the new regime — no exemptions to declare.</p>
                  ) : (
                    <div className="grid md:grid-cols-2 gap-2 text-[14px]">
                  ...
                  {status === 'rejected' && d.rejectionReason && (
                    <p className="text-[13px] text-rose-700 mt-3">Rejection note: {d.rejectionReason}</p>
                  )}
                </div>
              )}
            </div>
