    ...
    <div className="max-w-6xl mx-auto p-6 space-y-5">
      ...
      <div className="bg-white border border-slate-200 rounded-xl px-5 py-4 flex items-center justify-between flex-wrap gap-3">
        <div className="flex items-center gap-3">
          ...
          <p className="text-[13px] font-bold text-slate-800">Period</p>
        </div>
        ...
        {['bonus','overtime','deduction','other'].map(t => {
          ...
          return (
            <div key={t} className={`rounded-xl border border-slate-200 p-4 ${meta.bg}`}>
              <p className={`text-[10.5px] font-bold uppercase tracking-wider ${meta.fg}`}>{meta.label}</p>
              <p className="text-[18px] font-bold text-slate-800 mt-1">{fmtINR(sum)}</p>
              <p className="text-[11px] text-slate-500 mt-0.5">{sub.length} {sub.length === 1 ? 'entry' : 'entries'}</p>
            </div>
          );
        })}
      </div>
      ...
      <div className="bg-white border border-slate-200 rounded-xl overflow-hidden">
        <div className="px-4 py-3 border-b border-slate-100 flex items-center justify-between">
          <h3 className="text-[14px] font-bold text-slate-800">Entries for {MONTH_NAMES[month]} {year}</h3>
          <p className="text-[12px] font-semibold text-slate-700">Net: <span className="text-emerald-700">{fmtINR(total)}</span></p>
        </div>
        {loading ? (
            ...
            {rows.map(r => {
              ...
              return (
                <div key={r.id} className="px-4 py-3 flex items-center justify-between hover:bg-slate-50">
                  <div className="flex items-center gap-3">
                    <span className={`px-2 py-0.5 rounded-full text-[11px] font-bold ${meta.bg} ${meta.fg}`}>{meta.label}</span>
                    <div>
                      <p className="text-[13px] font-bold text-slate-800">{r.firstName} {r.lastName}</p>
                      <p className="text-[11px] text-slate-500">{r.employeeCode}{r.reason ? ` · ${r.reason}` : ''}</p>
                    </div>
                  </div>
                  <div className="flex items-center gap-4">
                    <p className={`text-[14px] font-bold ${r.type === 'deduction' ? 'text-rose-700' : 'text-emerald-700'}`}>
    ...
    <div className="fixed inset-0 z-50 bg-black/50 flex items-center justify-center p-4">
      <div className="bg-white rounded-2xl w-full max-w-md shadow-2xl overflow-hidden">
        <div className="bg-gradient-to-r from-amber-500 to-orange-500 text-white px-6 py-4">
          <p className="text-[11px] uppercase tracking-wider font-bold opacity-90">Add Adjustment</p>
          <p className="text-[16px] font-bold mt-0.5">{MONTH_NAMES[month]} {year}</p>
        </div>
        <div className="p-5 space-y-3">
            ...
            <div className="grid grid-cols-4 gap-2 mt-1">
              {Object.entries(TYPE_META).map(([k, m]) => (
                <button key={k} onClick={() => setType(k)}
                  className={`px-2 py-1.5 rounded-lg text-[12px] font-semibold border transition-all ${
                    type === k
          ...
          <div>
            <label className="text-[11.5px] font-bold text-slate-600 uppercase tracking-wider">Amount (₹)</label>
            <div className="mt-1 flex items-center bg-slate-50 border border-slate-200 rounded-lg px-3">
              <IndianRupee size={14} className="text-slate-400" />
              <input type="number" value={amount} onChange={e => setAmount(e.target.value)}
                placeholder="0" className="flex-1 bg-transparent py-2 text-[13px] focus:outline-none" />
            </div>
          </div>
          <div>
            <label className="text-[11.5px] font-bold text-slate-600 uppercase tracking-wider">Reason (optional)</label>
