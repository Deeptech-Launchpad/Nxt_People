...
function PanelCard({ icon: Icon, title, children }) {
  return (
    <div className="bg-white rounded-lg border border-slate-200 p-4 shadow-sm">
      ...
      <div className="text-[11.5px] text-slate-600">{children}</div>
    </div>
  );
}
  ...
  return (
    <div className="flex items-center gap-2 py-1">
      {p.photoUrl ? (
        ...
        <div className="w-7 h-7 rounded-full bg-slate-100 text-slate-500 flex items-center justify-center text-[10px] font-bold border border-slate-200">
      ...
      <div className="min-w-0">
        ...
        {p.workLocation && <p className="text-[10.5px] text-slate-400 truncate">{p.workLocation}</p>}
        {p.designation  && <p className="text-[10.5px] text-slate-400 truncate">{p.designation}</p>}
      </div>
    </div>
  );
}
    ...
    <div className="flex gap-5 p-6 min-h-[calc(100vh-100px)]">
      ...
      <div className="flex-1 min-w-0 bg-white rounded-lg border border-slate-200 shadow-sm overflow-hidden flex flex-col">
        ...
        <div className="px-5 py-4 border-b border-slate-100 flex items-center gap-4">
          <div className="w-12 h-12 rounded-lg bg-slate-100 text-slate-700 font-bold flex items-center justify-center text-[16px]">
            {(deptName || 'D').charAt(0).toUpperCase()}
          </div>
          <div>
            <p className="text-[13px] text-slate-400 leading-tight">Department</p>
            <p className="text-[15px] font-bold text-slate-800">{deptName}</p>
          </div>
        </div>
        ...
        <PanelCard icon={Users} title="Team Strength">
          {loading ? '—' : (
            <div className="flex items-center justify-between">
              ...
              <span className="text-[14px] font-bold text-slate-800">{data?.teamStrength ?? 0}</span>
            </div>
          )}
        </PanelCard>
            ...
            <div className="space-y-1">
              {(data?.locationDiversity || []).map(l => (
                <div key={l.location} className="flex items-center justify-between">
                  ...
                  <span className="text-[11px] font-bold text-slate-800 ml-2">{l.n}</span>
                </div>
              ))}
            </div>
          )}
...
function Row({ dot, label, value }) {
  return (
    <div className="flex items-center justify-between">
      ...
      <span className="text-[11px] font-bold text-slate-800">{value || 0}</span>
    </div>
  );
}
