        ...
        <div className="p-5 border-b border-slate-100 flex flex-wrap gap-3 items-center justify-between">
          <div className="flex flex-wrap gap-3 flex-1">
            <div className="relative min-w-48 flex-1 max-w-72">
              ...
              <input value={search} onChange={e=>{setSearch(e.target.value);setPage(1)}} placeholder="Search employees..." className="w-full pl-9 pr-4 py-2.5 text-base border border-slate-200 rounded-xl focus:outline-none focus:border-brand-400 focus:ring-1 focus:ring-brand-400"/>
            </div>
            <select value={deptFilter} onChange={e=>{setDeptFilter(e.target.value);setPage(1)}} className="px-3 py-2.5 text-base border border-slate-200 rounded-xl focus:outline-none focus:border-brand-400">
              <option value="">All Departments</option>{departments.map(d=><option key={d}>{d}</option>)}
            </select>
            <select value={desigFilter} onChange={e=>{setDesigFilter(e.target.value);setPage(1)}} className="px-3 py-2.5 text-base border border-slate-200 rounded-xl focus:outline-none focus:border-brand-400">
              <option value="">All Designations</option>{designations.map(d=><option key={d}>{d}</option>)}
            </select>
            <select value={roleFilter} onChange={e=>{setRoleFilter(e.target.value);setPage(1)}} className="px-3 py-2.5 text-base border border-slate-200 rounded-xl focus:outline-none focus:border-brand-400">
          ...
          {isAdmin && (
            <button
              ...
              className="flex items-center gap-2 bg-amber-50 hover:bg-amber-100 text-amber-700 border border-amber-200 px-4 py-2.5 rounded-xl text-base font-medium transition-colors disabled:opacity-60"
            >
          ...
          <button onClick={() => setOnboardingModal(true)} className="flex items-center gap-2 bg-white hover:bg-slate-50 text-brand-600 border border-brand-200 px-4 py-2.5 rounded-xl text-base font-medium transition-colors">
                    ...
                    <tr key={emp._id} className="hover:bg-slate-50 transition-colors">
                      <td className="px-5 py-3.5">
                        <div className="flex items-center gap-3">
                          ...
                          <div className="w-9 h-9 rounded-full bg-gradient-to-br from-brand-400 to-brand-600 flex items-center justify-center text-white text-sm font-bold flex-shrink-0 relative overflow-hidden">
                          ...
                          <div>
                            <p className="text-base font-medium text-slate-700 flex items-center gap-1.5">
                              ...
                              {emp.isBlacklisted && (
                                <span title="Blacklisted — flag during future onboarding" className="text-[11px] font-bold px-1.5 py-0.5 rounded bg-red-100 text-red-700">🚫 BLACKLIST</span>
                              )}
                              {emp.status === 'notice_period' && (
                                <span title={`Notice period ends ${emp.noticePeriodEndDate || ''}`} className="text-[11px] font-bold px-1.5 py-0.5 rounded bg-amber-100 text-amber-700">NOTICE</span>
                              )}
                            </p>
                      ...
                      <td className="px-5 py-3.5 text-base text-slate-500 font-mono">{emp.employeeId}</td>
                      <td className="px-5 py-3.5 text-base text-slate-600">{emp.department}</td>
                      <td className="px-5 py-3.5"><span className={`px-2.5 py-1 rounded-full text-sm font-medium ${roleColors[emp.role]}`}>{roleLabel(emp.role)}</span></td>
                      <td className="px-5 py-3.5 text-base text-slate-600">{emp.designation}</td>
                      <td className="px-5 py-3.5 text-base text-slate-500">
                      ...
                      <td className="px-5 py-3.5">
                        <div className="flex items-center gap-2">
                          ...
                          <button onClick={()=>openStatus(emp)} title="Update employment status (notice period / exit / blacklist)" className="w-8 h-8 flex items-center justify-center rounded-lg bg-amber-50 text-amber-600 hover:bg-amber-100 transition-colors text-[17px] font-bold">⚑</button>
        ...
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl w-full max-w-md shadow-2xl">
            <div className="flex items-center justify-between p-6 border-b border-slate-100">
              <div>
                <h3 className="font-display font-semibold text-slate-800 text-xl">Send Preboarding Email</h3>
                <p className="text-sm text-slate-400 mt-0.5">Invite a candidate to complete their registration</p>
              </div>
            ...
            <form onSubmit={handleSendOnboarding} className="p-6 space-y-4">
              <div>
                <label className="block text-sm font-medium text-slate-600 mb-1.5">Candidate Email <span className="text-brand-600">*</span></label>
                <input
                  type="email"
                  ...
                  required
                  className="w-full border border-slate-200 rounded-xl px-3 py-2.5 text-base focus:outline-none focus:border-brand-400 focus:ring-1 focus:ring-brand-400"
                />
              </div>
              <div>
                <label className="block text-sm font-medium text-slate-600 mb-1.5">Candidate Name <span className="text-slate-400">(optional)</span></label>
                <input
                  type="text"
              ...
              <div>
                <label className="block text-sm font-medium text-slate-600 mb-1.5">Registration Deadline <span className="text-slate-400">(optional)</span></label>
                <input
                  type="date"
              ...
              <div className="flex gap-3 pt-1">
                <button type="button" onClick={() => setOnboardingModal(false)} className="flex-1 border border-slate-200 text-slate-600 py-2.5 rounded-xl text-base font-medium hover:bg-slate-50 transition-colors">Cancel</button>
                <button type="submit" disabled={sendingEmail} className="flex-1 flex items-center justify-center gap-2 bg-brand-600 hover:bg-brand-500 text-white py-2.5 rounded-xl text-base font-medium transition-colors disabled:opacity-60">
        ...
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl w-full max-w-lg shadow-2xl max-h-[90vh] flex flex-col">
            <div className="flex items-center justify-between p-6 border-b border-slate-100 flex-shrink-0">
              <h3 className="font-display font-semibold text-slate-800 text-xl">{editEmp ? 'Edit Employee' : 'Add New Employee'}</h3>
              ...
              <div className="grid grid-cols-2 gap-4">
                {[['firstName','First Name'],['lastName','Last Name']].map(([f,l])=>(
                  <div key={f}>
                    ...
                    <input value={form[f]} onChange={e=>setForm({...form,[f]:e.target.value})} required className="w-full border border-slate-200 rounded-xl px-3 py-2.5 text-base focus:outline-none focus:border-brand-400 focus:ring-1 focus:ring-brand-400"/>
                  </div>
                ))}
              </div>
              <div>
                <label className="block text-sm font-medium text-slate-600 mb-1.5">Email</label>
                <input type="email" value={form.email} onChange={e=>setForm({...form,email:e.target.value})} required className="w-full border border-slate-200 rounded-xl px-3 py-2.5 text-base focus:outline-none focus:border-brand-400 focus:ring-1 focus:ring-brand-400"/>
              </div>
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-sm font-medium text-slate-600 mb-1.5">Company</label>
                  <select value={form.company} onChange={e=>setForm({...form,company:e.target.value})} className="w-full border border-slate-200 rounded-xl px-3 py-2.5 text-base focus:outline-none focus:border-brand-400">
                    <option value="">Select...</option>
                    {COMPANIES.map(c => <option key={c} value={c}>{c}</option>)}
                  </select>
                </div>
                <div>
                  <label className="block text-sm font-medium text-slate-600 mb-1.5">Employee ID</label>
                  <input value={form.employeeId} onChange={e=>setForm({...form,employeeId:e.target.value})} placeholder="e.g. NXT0001" className="w-full border border-slate-200 rounded-xl px-3 py-2.5 text-base font-mono focus:outline-none focus:border-brand-400 focus:ring-1 focus:ring-brand-400"/>
                  <p className="text-[13px] text-slate-400 mt-1">{editEmp ? 'Change if needed — must be unique.' : 'Auto-suggested based on Company.'}</p>
                </div>
              </div>
              {!editEmp && <div>
                <label className="block text-sm font-medium text-slate-600 mb-1.5">Password</label>
                <input type="password" value={form.password} onChange={e=>setForm({...form,password:e.target.value})} required={!editEmp} className="w-full border border-slate-200 rounded-xl px-3 py-2.5 text-base focus:outline-none focus:border-brand-400 focus:ring-1 focus:ring-brand-400"/>
              </div>}
<div className="grid grid-cols-2 gap-4">
                 <div>
                 ...
                 <div>
                   ...
                   <select value={form.role} onChange={e=>setForm({...form,role:e.target.value})} className="w-full border border-slate-200 rounded-xl px-3 py-2.5 text-base focus:outline-none focus:border-brand-400">
               ...
               <div className="grid grid-cols-2 gap-4">
                 <div>
                   <label className="block text-sm font-medium text-slate-600 mb-1.5">Designation</label>
                   <select value={form.designation} onChange={e=>setForm({...form,designation:e.target.value})} className="w-full border border-slate-200 rounded-xl px-3 py-2.5 text-base focus:outline-none focus:border-brand-400">
                     <option value="">Select...</option>
                     {designations.map(d=><option key={d}>{d}</option>)}
                   </select>
                 </div>
                 <div>
                   <label className="block text-sm font-medium text-slate-600 mb-1.5">Reporting Person</label>
                   <select value={form.reportingManagerId} onChange={e=>setForm({...form,reportingManagerId:e.target.value})} className="w-full border border-slate-200 rounded-xl px-3 py-2.5 text-base focus:outline-none focus:border-brand-400">
               ...
               <div className="grid grid-cols-2 gap-4">
                 <div>
                   <label className="block text-sm font-medium text-slate-600 mb-1.5">Secondary Reporting Person</label>
                   <select value={form.approvingAuthorityId} onChange={e=>setForm({...form,approvingAuthorityId:e.target.value})} className="w-full border border-slate-200 rounded-xl px-3 py-2.5 text-base focus:outline-none focus:border-brand-400">
                     <option value="">None</option>
               ...
               <div className="grid grid-cols-2 gap-4">
                 <div>
                   ...
                   <input type="date" value={form.joiningDate} onChange={e=>setForm({...form,joiningDate:e.target.value})} className="w-full border border-slate-200 rounded-xl px-3 py-2.5 text-base focus:outline-none focus:border-brand-400"/>
                 </div>
                 <div>
                   <label className="block text-sm font-medium text-slate-600 mb-1.5">Phone</label>
                   <input value={form.phone} onChange={e=>setForm({...form,phone:e.target.value})} className="w-full border border-slate-200 rounded-xl px-3 py-2.5 text-base focus:outline-none focus:border-brand-400"/>
                 </div>
               </div>
              {/* Personal */}
              <div className="border-t border-slate-100 pt-4">
                <p className="text-sm font-semibold text-slate-500 uppercase tracking-wider mb-3">Personal</p>
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    <label className="block text-sm font-medium text-slate-600 mb-1.5">Nickname</label>
                    <input value={form.nickName} onChange={e=>setForm({...form,nickName:e.target.value})} className="w-full border border-slate-200 rounded-xl px-3 py-2.5 text-base focus:outline-none focus:border-brand-400"/>
                  </div>
                  <div>
                    <label className="block text-sm font-medium text-slate-600 mb-1.5">Date of Birth</label>
                    <input type="date" value={form.dateOfBirth} onChange={e=>setForm({...form,dateOfBirth:e.target.value})} className="w-full border border-slate-200 rounded-xl px-3 py-2.5 text-base focus:outline-none focus:border-brand-400"/>
                  </div>
                  <div>
                    <label className="block text-sm font-medium text-slate-600 mb-1.5">Gender</label>
                    <select value={form.gender} onChange={e=>setForm({...form,gender:e.target.value})} className="w-full border border-slate-200 rounded-xl px-3 py-2.5 text-base focus:outline-none focus:border-brand-400">
                      <option value="">Select...</option>
                  ...
                  <div>
                    ...
                    <select value={form.maritalStatus} onChange={e=>setForm({...form,maritalStatus:e.target.value})} className="w-full border border-slate-200 rounded-xl px-3 py-2.5 text-base focus:outline-none focus:border-brand-400">
                      <option value="">Select...</option>
                      {['Single','Married'].map(o=><option key={o}>{o}</option>)}
                    </select>
                  </div>
                  <div>
                    <label className="block text-sm font-medium text-slate-600 mb-1.5">Blood Group</label>
                    <select value={form.bloodGroup} onChange={e=>setForm({...form,bloodGroup:e.target.value})} className="w-full border border-slate-200 rounded-xl px-3 py-2.5 text-base focus:outline-none focus:border-brand-400">
                      <option value="">Select...</option>
                  ...
                  <div>
                    ...
                    <input value={form.nationality} onChange={e=>setForm({...form,nationality:e.target.value})} placeholder="e.g. Indian" className="w-full border border-slate-200 rounded-xl px-3 py-2.5 text-base focus:outline-none focus:border-brand-400"/>
                  </div>
                </div>
              </div>
              ...
              {/* Contact */}
              <div className="border-t border-slate-100 pt-4">
                <p className="text-sm font-semibold text-slate-500 uppercase tracking-wider mb-3">Contact</p>
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    <label className="block text-sm font-medium text-slate-600 mb-1.5">Personal Email</label>
                    <input type="email" value={form.personalEmail} onChange={e=>setForm({...form,personalEmail:e.target.value})} className="w-full border border-slate-200 rounded-xl px-3 py-2.5 text-base focus:outline-none focus:border-brand-400"/>
                  </div>
                  <div>
                    <label className="block text-sm font-medium text-slate-600 mb-1.5">Work Phone</label>
                    <input value={form.workPhone} onChange={e=>setForm({...form,workPhone:e.target.value})} className="w-full border border-slate-200 rounded-xl px-3 py-2.5 text-base focus:outline-none focus:border-brand-400"/>
                  </div>
                  <div className="col-span-2">
                    <label className="block text-sm font-medium text-slate-600 mb-1.5">Current Address</label>
                    <textarea rows={2} value={form.address} onChange={e=>setForm({...form,address:e.target.value})} className="w-full border border-slate-200 rounded-xl px-3 py-2.5 text-base focus:outline-none focus:border-brand-400 resize-none"/>
                  </div>
                  <div className="col-span-2">
                    <label className="block text-sm font-medium text-slate-600 mb-1.5">Permanent Address</label>
                    <textarea rows={2} value={form.permanentAddress} onChange={e=>setForm({...form,permanentAddress:e.target.value})} className="w-full border border-slate-200 rounded-xl px-3 py-2.5 text-base focus:outline-none focus:border-brand-400 resize-none"/>
                  </div>
                </div>
              </div>
              ...
              <div className="border-t border-slate-100 pt-4">
                ...
                <div className="grid grid-cols-3 gap-3">
                  <div>
                    ...
                    <input value={form.emergencyContactName} onChange={e=>setForm({...form,emergencyContactName:e.target.value})} className="w-full border border-slate-200 rounded-xl px-3 py-2.5 text-base focus:outline-none focus:border-brand-400"/>
                  </div>
                  <div>
                    <label className="block text-sm font-medium text-slate-600 mb-1.5">Phone</label>
                    <input value={form.emergencyContactPhone} onChange={e=>setForm({...form,emergencyContactPhone:e.target.value})} className="w-full border border-slate-200 rounded-xl px-3 py-2.5 text-base focus:outline-none focus:border-brand-400"/>
                  </div>
                  <div>
                    <label className="block text-sm font-medium text-slate-600 mb-1.5">Relationship</label>
                    <input value={form.emergencyContactRelation} onChange={e=>setForm({...form,emergencyContactRelation:e.target.value})} placeholder="e.g. Father" className="w-full border border-slate-200 rounded-xl px-3 py-2.5 text-base focus:outline-none focus:border-brand-400"/>
                  </div>
                </div>
              </div>
              ...
              <div className="border-t border-slate-100 pt-4">
                <p className="text-sm font-semibold text-slate-500 uppercase tracking-wider mb-3">Experience & Expertise</p>
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    <label className="block text-sm font-medium text-slate-600 mb-1.5">Total Experience</label>
                    <input value={form.totalExperience} onChange={e=>setForm({...form,totalExperience:e.target.value})} placeholder="e.g. 5 years 3 months" className="w-full border border-slate-200 rounded-xl px-3 py-2.5 text-base focus:outline-none focus:border-brand-400"/>
                  </div>
                  <div>
                  ...
                  <div className="col-span-2">
                    ...
                    <textarea rows={2} value={form.expertise} onChange={e=>setForm({...form,expertise:e.target.value})} placeholder="e.g. React, Node.js, Python (comma separated)" className="w-full border border-slate-200 rounded-xl px-3 py-2.5 text-base focus:outline-none focus:border-brand-400 resize-none"/>
                  </div>
                </div>
              </div>
              ...
              {editEmp && (
                <div className="border-t border-slate-100 pt-4">
                  <p className="text-sm font-semibold text-slate-500 uppercase tracking-wider mb-3">Employment Status</p>
                  <div className="grid grid-cols-2 gap-3">
                    <div>
                    ...
                    <div>
                      <label className="block text-sm font-medium text-slate-600 mb-1.5">Employment Type</label>
                      <select value={form.employmentType} onChange={e=>setForm({...form,employmentType:e.target.value})} className="w-full border border-slate-200 rounded-xl px-3 py-2.5 text-base focus:outline-none focus:border-brand-400">
                        <option value="">Select...</option>
                    ...
                    {form.status !== 'active' && (
                      <div className="col-span-2">
                        <label className="block text-sm font-medium text-slate-600 mb-1.5">Exit Date <span className="text-slate-400 font-normal">(when they left)</span></label>
                        <input type="date" value={form.exitDate} onChange={e=>setForm({...form,exitDate:e.target.value})} className="w-full border border-slate-200 rounded-xl px-3 py-2.5 text-base focus:outline-none focus:border-brand-400"/>
                      </div>
                    )}
                  </div>
                </div>
              ...
              {editEmp && (
                <div className="border-t border-slate-100 pt-4">
                  <p className="text-sm font-semibold text-slate-500 uppercase tracking-wider mb-3">App Access</p>
                  <AppAccessPicker
                    apps={appAccessList}
      ...
      {statusModal && (
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl w-full max-w-lg shadow-2xl max-h-[90vh] flex flex-col">
            <div className="flex items-center justify-between p-6 border-b border-slate-100">
              <div>
                <h3 className="font-display font-semibold text-slate-800 text-xl">Update Employment Status</h3>
                <p className="text-sm text-slate-400 mt-0.5">{statusModal.firstName} {statusModal.lastName} · {statusModal.employeeId || statusModal.email}</p>
              </div>
            ...
            <form onSubmit={(e)=>{e.preventDefault();handleStatusSave();}} className="p-6 space-y-4 overflow-y-auto flex-1">
              <div>
                <label className="block text-sm font-medium text-slate-600 mb-1.5">New Status</label>
                <select value={statusForm.status} onChange={e=>setStatusForm({...statusForm,status:e.target.value})} className="w-full border border-slate-200 rounded-xl px-3 py-2.5 text-base focus:outline-none focus:border-brand-400">
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="block text-sm font-medium text-slate-600 mb-1.5">Status Applied Date</label>
                  <input type="date" value={statusForm.statusAppliedAt} onChange={e=>setStatusForm({...statusForm,statusAppliedAt:e.target.value})} required className="w-full border border-slate-200 rounded-xl px-3 py-2.5 text-base focus:outline-none focus:border-brand-400"/>
                </div>
                {statusForm.status === 'notice_period' && (
                  <div>
                    <label className="block text-sm font-medium text-slate-600 mb-1.5">Notice Period End Date <span className="text-red-500">*</span></label>
                    <input type="date" value={statusForm.noticePeriodEndDate} onChange={e=>setStatusForm({...statusForm,noticePeriodEndDate:e.target.value})} required min={statusForm.statusAppliedAt || undefined} className="w-full border border-slate-200 rounded-xl px-3 py-2.5 text-base focus:outline-none focus:border-brand-400"/>
                    <p className="text-[12px] text-slate-400 mt-1">Login access auto-revoked the day after this date.</p>
                  </div>
                )}
              </div>
              ...
              <div>
                ...
                <textarea rows={3} value={statusForm.statusReason} onChange={e=>setStatusForm({...statusForm,statusReason:e.target.value})} placeholder="Why this status change?" className="w-full border border-slate-200 rounded-xl px-3 py-2.5 text-base focus:outline-none focus:border-brand-400 resize-none"/>
              </div>

              {['notice_period','resigned','terminated','inactive'].includes(statusForm.status) && (
                <div>
                  <label className="block text-sm font-medium text-slate-600 mb-1.5">Will you rehire?</label>
                  <div className="flex gap-3">
                    {[
                      ...
                      <label key={o.v} className={`flex-1 px-3 py-2 rounded-xl border text-base font-medium text-center cursor-pointer ${statusForm.rehireEligibility === o.v ? o.cls : 'border-slate-200 text-slate-500 hover:bg-slate-50'}`}>
              ...
              <div>
                <label className="flex items-center gap-3 cursor-pointer">
                  ...
                  <div>
                    <p className="text-base font-medium text-slate-700">Blacklist this person</p>
                    <p className="text-[13px] text-slate-400">If checked, future onboarding for the same email shows a warning. Use sparingly — this is a permanent mark on the record.</p>
                  </div>
                </label>
              </div>

              <div className="flex gap-3 pt-2">
                <button type="button" onClick={()=>setStatusModal(null)} className="flex-1 border border-slate-200 text-slate-600 py-2.5 rounded-xl text-base font-medium hover:bg-slate-50 transition-colors">Cancel</button>
                <button type="submit" disabled={statusSaving} className="flex-1 bg-brand-600 hover:bg-brand-500 text-white py-2.5 rounded-xl text-base font-medium transition-colors disabled:opacity-60">
          ...
          <div className="bg-white rounded-2xl w-full max-w-4xl max-h-[90vh] flex flex-col shadow-2xl">
            <div className="flex items-center justify-between p-6 border-b border-slate-100 flex-shrink-0">
              <div>
                ...
                <p className="text-slate-400 text-base mt-0.5">{viewEmpData ? `${viewEmpData.firstName} ${viewEmpData.lastName}` : 'Loading...'}</p>
              </div>
                ...
                <div className="space-y-6">
                  ...
                  <div className="bg-white rounded-xl p-5 border border-slate-200 shadow-sm">
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-base">
                      <div><span className="block text-slate-400 text-sm font-medium mb-0.5">Employee ID</span><span className="text-slate-800 font-mono font-medium">{viewEmpData.employeeId || '—'}</span></div>
                      <div><span className="block text-slate-400 text-sm font-medium mb-0.5">Email</span><span className="text-slate-800 break-all">{viewEmpData.email}</span></div>
                      <div><span className="block text-slate-400 text-sm font-medium mb-0.5">Department</span><span className="text-slate-800">{viewEmpData.department || '—'}</span></div>
                      <div><span className="block text-slate-400 text-sm font-medium mb-0.5">Designation</span><span className="text-slate-800">{viewEmpData.designation || '—'}</span></div>
                      <div><span className="block text-slate-400 text-sm font-medium mb-0.5">Role</span><span className="text-slate-800">{roleLabel(viewEmpData.role)}</span></div>
                      <div><span className="block text-slate-400 text-sm font-medium mb-0.5">Joining Date</span><span className="text-slate-800">{viewEmpData.joiningDate ? new Date(viewEmpData.joiningDate).toLocaleDateString() : '—'}</span></div>
                      <div><span className="block text-slate-400 text-sm font-medium mb-0.5">Company</span><span className="text-slate-800">{viewEmpData.company || '—'}</span></div>
                      <div><span className="block text-slate-400 text-sm font-medium mb-0.5">Division</span><span className="text-slate-800">{viewEmpData.division || '—'}</span></div>
                    </div>
                  </div>
                    ...
                    <div className="bg-white rounded-xl p-5 border border-slate-200 shadow-sm">
                      ...
                      <div className="space-y-3">
                        {viewEmpData.education.map((edu, i) => (
                          <div key={i} className="bg-slate-50 p-3 rounded-lg border border-slate-100 text-base">
            ...
            <div className="p-5 border-t border-slate-100 bg-white rounded-b-2xl flex justify-end">
              <button onClick={() => { setViewEmpId(null); setViewEmpData(null); }} className="px-6 py-2.5 bg-slate-100 hover:bg-slate-200 text-slate-700 font-medium rounded-xl text-base transition-colors">Close</button>
            </div>
          </div>
        </div>
      )}
          ...
          <div className="bg-white rounded-2xl w-full max-w-md shadow-2xl">
            <div className="flex items-center justify-between p-5 border-b border-slate-100">
              <div className="flex items-center gap-2">
                ...
                <h3 className="font-semibold text-slate-800 text-[17px]">Zoho sync complete</h3>
              </div>
            ...
            <div className="p-5 space-y-3">
              <div className="grid grid-cols-2 gap-3">
                {[
                  ...
                  <div key={t.label} className="bg-slate-50 border border-slate-100 rounded-lg p-3">
                    <p className="text-[13px] font-semibold text-slate-500 uppercase">{t.label}</p>
              ...
              {zohoResult.errors?.length > 0 && (
                <div className="border border-red-200 bg-red-50/60 rounded-lg p-3">
                  ...
                  <ul className="text-[13px] text-red-700 space-y-1 max-h-32 overflow-y-auto">
            ...
            <div className="p-4 border-t border-slate-100 flex justify-end">
              <button
                ...
                className="px-5 py-2 bg-brand-600 hover:bg-brand-500 text-white text-base font-semibold rounded-xl"
              >
                Done
              </button>
            </div>
