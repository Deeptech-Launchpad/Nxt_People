  ...
  return (
    <div className="bg-white rounded-xl shadow-sm border border-slate-200 flex flex-col min-h-[calc(100vh-12rem)]">
      ...
      <div className="px-6 py-4 border-b border-slate-100 flex items-center justify-between gap-4">
        <div className="flex items-center gap-3 min-w-0">
          ...
          <div className="flex items-center gap-2">
            ...
            <div>
              <h2 className="text-[15px] font-bold text-slate-800">Permission Usage</h2>
              <p className="text-[12px] text-slate-400">Monthly permission hours · 4h allowance per employee · no carry-forward</p>
            </div>
          </div>
        </div>
        {/* Month switcher */}
        <div className="flex items-center gap-2">
          ...
          <span className="text-[13px] font-semibold text-slate-700 w-36 text-center">{MONTHS[month - 1]} {year}</span>
      ...
      <div className="px-6 py-3 border-b border-slate-100 bg-slate-50/50 flex flex-wrap gap-6">
        <div><span className="text-[11px] text-slate-400 uppercase tracking-wide font-semibold">Employees with usage</span><p className="text-[15px] font-bold text-slate-800">{rows.length}</p></div>
        <div><span className="text-[11px] text-slate-400 uppercase tracking-wide font-semibold">Approved hours</span><p className="text-[15px] font-bold text-emerald-600">{totalApproved.toFixed(2)}h</p></div>
        <div><span className="text-[11px] text-slate-400 uppercase tracking-wide font-semibold">Pending hours</span><p className="text-[15px] font-bold text-amber-600">{totalPending.toFixed(2)}h</p></div>
      </div>
        ...
        <table className="w-full text-left">
          ...
          <tbody className="divide-y divide-slate-50">
            {loading ? (
              ...
              <tr><td colSpan={7} className="px-5 py-16 text-center text-slate-400 text-[13px]">No permission requests in {MONTHS[month - 1]} {year}</td></tr>
            ) : rows.map(r => {
              ...
              return (
                <tr key={r._id} className="hover:bg-slate-50/70 transition-colors">
                  <td className="px-5 py-3.5">
                    <p className="text-[13px] font-semibold text-slate-700">{r.firstName} {r.lastName}</p>
                    <p className="text-[11px] text-slate-400 font-mono">{r.employeeId}</p>
                  </td>
                  <td className="px-5 py-3.5 text-[12.5px] text-slate-600">{r.department || '—'}</td>
                  <td className="px-5 py-3.5 text-center text-[13px] text-slate-600">{r.requests}</td>
                  <td className="px-5 py-3.5 text-center text-[13px] font-semibold text-emerald-600">{approved.toFixed(2)}h</td>
                  <td className="px-5 py-3.5 text-center text-[13px] font-semibold text-amber-600">{pending.toFixed(2)}h</td>
                  <td className="px-5 py-3.5 text-center text-[13px] font-semibold text-slate-700">{(parseFloat(r.remainingHours) || 0).toFixed(2)}h</td>
                  <td className="px-5 py-3.5">
