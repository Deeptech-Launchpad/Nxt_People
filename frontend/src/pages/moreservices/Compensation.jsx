  ...
  return (
    <div className="p-6 max-w-5xl space-y-5">
      <div className="flex items-center justify-between">
        <h2 className="text-[17px] font-bold text-slate-800">Compensation Claims</h2>
        ...
        <div className="flex gap-1 bg-slate-100 p-1 rounded-lg w-fit ml-auto">
          {STATUS_TABS.map(t => (
            <button key={t} onClick={() => setTab(t)}
              className={`px-3 py-1.5 rounded-md text-[14px] font-medium transition-all ${tab === t ? 'bg-white shadow-sm text-slate-800' : 'text-slate-500 hover:text-slate-700'}`}>
              {t}
            </button>
          ))}
        </div>
              ...
              <tr key={c._id} className="hover:bg-slate-50 transition-colors">
                {view === 'All (Admin)' && (
                  <td className="px-4 py-3 text-[14px] text-slate-700">
                    ...
                    <span className="block text-[12px] text-slate-400">{c.employee?.department}</span>
                  </td>
                )}
                <td className="px-4 py-3 text-[14px] font-medium text-slate-800">{c.claimType}</td>
                <td className="px-4 py-3 text-[14px] font-bold text-slate-700">{fmtMoney(c.amount)}</td>
                <td className="px-4 py-3 text-[14px] text-slate-600">{fmtDate(c.claimDate)}</td>
                <td className="px-4 py-3 text-[14px] text-slate-600 max-w-[200px] truncate" title={c.description}>{c.description || '—'}</td>
                ...
                <td className="px-4 py-3">
                  <span className={`text-[13px] font-semibold px-2 py-0.5 rounded-full capitalize ${STATUS_COLOR[c.status] || 'bg-slate-100'}`}>{c.status}</span>
                  {c.status === 'rejected' && c.rejectionReason && (
                    <p className="text-[12px] text-red-500 mt-0.5 max-w-[160px]" title={c.rejectionReason}>{c.rejectionReason}</p>
                  )}
                </td>
                <td className="px-4 py-3">
        ...
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-xl w-full max-w-md shadow-2xl">
            ...
            <form onSubmit={handleSubmit} className="p-5 space-y-4">
              <div>
                <label className="block text-[14px] font-medium text-slate-600 mb-1.5">Type *</label>
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="block text-[14px] font-medium text-slate-600 mb-1.5">Amount (₹) *</label>
                  <input type="number" min="0" step="0.01" value={form.amount}
                    onChange={e => setForm({ ...form, amount: e.target.value })} required
                    className="w-full border border-slate-200 rounded-lg px-3 py-2 text-[14px] focus:outline-none focus:border-blue-400"/>
                </div>
                <div>
                  <label className="block text-[14px] font-medium text-slate-600 mb-1.5">Claim Date *</label>
              ...
              <div>
                <label className="block text-[14px] font-medium text-slate-600 mb-1.5">Description</label>
              ...
              <div>
                <label className="block text-[14px] font-medium text-slate-600 mb-1.5">Receipt <span className="text-slate-400 font-normal">(PDF / image, max 5MB, optional)</span></label>
                <label className="border-2 border-dashed border-slate-200 rounded-lg p-4 text-center hover:border-blue-300 transition-colors cursor-pointer block">
                  {form.receipt ? (
                    <div><FileText size={20} className="text-blue-500 mx-auto mb-1"/><p className="text-[14px] font-medium text-slate-700">{form.receipt.name}</p></div>
                  ) : (
                    <div><Upload size={20} className="text-slate-300 mx-auto mb-1"/><p className="text-[14px] text-slate-400">Click to attach</p></div>
                  )}
