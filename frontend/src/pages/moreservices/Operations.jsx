  ...
  return (
    <div className="bg-white rounded-xl shadow-sm border border-slate-200 min-h-[calc(100vh-12rem)]">
      <div className="px-6 py-5 border-b border-slate-100">
        <h2 className="text-[16px] font-semibold text-slate-900">Services</h2>
        <p className="text-[13px] text-slate-500 mt-1">Operations workspace — open <span className="font-semibold text-blue-700">Leave Tracker</span> to manage all leave requests.</p>
      </div>
          ...
          {SERVICES.map(({ key, label, icon: Icon, color }) => {
            ...
            return (
              ...
              >
                ...
                <span className={`text-[12px] font-semibold text-center leading-tight ${functional ? 'text-slate-900' : 'text-slate-400'}`}>
                  {label}
                </span>
                {functional && (
