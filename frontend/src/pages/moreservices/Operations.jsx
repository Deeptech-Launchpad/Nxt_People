  ...
  return (
    <div className="bg-white rounded-xl shadow-sm border border-slate-200 min-h-[calc(100vh-12rem)]">
      <div className="px-6 py-4 border-b border-slate-100">
        <h2 className="text-[15px] font-bold text-slate-800">Services</h2>
        <p className="text-[12px] text-slate-400 mt-0.5">Operations workspace — open <span className="font-semibold text-blue-600">Leave Tracker</span> to manage all leave requests.</p>
      </div>
          ...
          {SERVICES.map(({ key, label, icon: Icon, color }) => {
            ...
            return (
              ...
              >
                ...
                <span className={`text-[12px] font-semibold text-center leading-tight ${functional ? 'text-slate-800' : 'text-slate-500'}`}>
                  {label}
                </span>
                {functional && (
                  <span className="text-[10px] font-bold text-blue-600 uppercase tracking-wide">Open</span>
                )}
              </button>
            );
          })}
