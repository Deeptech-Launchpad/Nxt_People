  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
      <div className="bg-white border border-slate-200 rounded-2xl p-6 w-full max-w-sm shadow-2xl">
        <h3 className="font-display font-bold text-slate-800 text-xl mb-5">{isEdit ? 'Edit Company' : 'Add Company'}</h3>
        <div className="space-y-4">
          <div>
            <label className="block text-sm font-medium text-slate-500 mb-1.5">Company name <span className="text-red-400">*</span></label>
            <input value={form.name} onChange={e => setForm(f => ({ ...f, name: e.target.value }))} className="w-full bg-white border border-slate-200 text-slate-800 px-3 py-2.5 rounded-xl text-base focus:outline-none focus:border-brand-500 placeholder-slate-400" placeholder="e.g. Acme Corporation" />
          </div>
          <div>
            <label className="block text-sm font-medium text-slate-500 mb-1.5">Short code</label>
            <input value={form.code} onChange={e => setForm(f => ({ ...f, code: e.target.value.toUpperCase() }))} className="w-full bg-white border border-slate-200 text-slate-800 px-3 py-2.5 rounded-xl text-base focus:outline-none focus:border-brand-500 placeholder-slate-400" placeholder="e.g. ACME" maxLength={8} />
          </div>
          <div>
            <label className="block text-sm font-medium text-slate-500 mb-1.5">Description</label>
            <textarea value={form.description} onChange={e => setForm(f => ({ ...f, description: e.target.value }))} rows={2} className="w-full bg-white border border-slate-200 text-slate-800 px-3 py-2.5 rounded-xl text-base focus:outline-none focus:border-brand-500 placeholder-slate-400 resize-none" placeholder="Optional description" />
          </div>
        </div>
...
function DivisionManager({ company, onUpdated }) {
  ...
  return (
    <div className="mt-4 pl-4 border-l-2 border-slate-200">
      <p className="text-sm font-semibold text-slate-500 uppercase tracking-wide mb-3">Divisions</p>
      <div className="space-y-1.5 mb-3">
        {(company.divisions || []).length === 0
          ? <p className="text-slate-600 text-sm italic">No divisions yet</p>
      ...
      <div className="flex gap-2">
        <input value={newDiv} onChange={e => setNewDiv(e.target.value)} onKeyDown={e => e.key === 'Enter' && addDivision()} placeholder="Add division…" className="flex-1 bg-white border border-slate-200 text-slate-800 px-3 py-2 rounded-lg text-base focus:outline-none focus:border-brand-500 placeholder-slate-400" />
        <button onClick={addDivision} disabled={loading || !newDiv.trim()} className="bg-brand-600/80 hover:bg-brand-600 text-slate-800 px-3 py-2 rounded-lg text-base transition-all disabled:opacity-50 flex items-center gap-1">
    ...
    <div className="p-6 max-w-4xl mx-auto">
      ...
      {loading ? (
        ...
        <div className="flex flex-col items-center justify-center py-16 bg-white shadow-sm border border-slate-200 rounded-2xl">
          ...
          <p className="text-slate-500 text-base mb-1">No companies configured yet.</p>
          <p className="text-slate-600 text-sm">Add your first company to allow users to register.</p>
        </div>
      ) : (
